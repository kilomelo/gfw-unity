...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        private long GetHuatuoSetting()
        {
            ...
            {
                using var parser = new TOMLParser(File.OpenText(HuatuoDefine.EditorConfigPath));
                if (parser.TryParse(out var rootNode, out var errors))
                {
                    ...
                    return node?.AsInteger?.Value ?? 0;
                }
                Debug.LogError("Load huatuo editor config failed with those error(s):");
            ...
            {
                Debug.LogError($"Huatuo config file [{HuatuoDefine.EditorConfigPath}] not found.");
            }
        ...
        private void SaveHuatuoSettings()
        {
            ...
            {
                using var parser = new TOMLParser(File.OpenText(HuatuoDefine.EditorConfigPath));
                if (parser.TryParse(out rootNode, out var errors))
                {
                    initialized = true;
                }
                else
            ...
            rootNode.WriteToFile(HuatuoDefine.EditorConfigPath);
        }

        // set build symbols basic on dynamic assembly platform setting
        private static bool ApplyHuatuoSymbols(long dynamicAssemblyPlatforms)
        {
        ...
        public void SelectDynamicAssemblyTarget(BuildTargetGroup targetGroup, bool selected)
        {
            if (selected)
            {
                DynamicAssemblyPlatforms |= (uint) (1 << (int)targetGroup);
            }
            else
            {
