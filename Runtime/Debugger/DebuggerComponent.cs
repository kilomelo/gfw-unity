...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            ...
            {
                if (!typeof(IDebuggerWindow).IsAssignableFrom(type) || type.IsAbstract || type.IsGenericType) continue;
                if (Attribute.GetCustomAttribute(type, typeof(CustomDebuggerWindowAttribute)) is CustomDebuggerWindowAttribute attri)
                    RegisterDebuggerWindow(attri.Path, Activator.CreateInstance(type) as IDebuggerWindow);
            }
